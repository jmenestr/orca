/* eslint-disable max-lines -- Why: Linear RPC dispatch stays in one adapter boundary so runtime issue-tracker wiring remains easy to scan. */
...
export type ResolvedRuntimeLinearWorktree = Worktree & {
...
export type RuntimeLinearCommandHost = {
  getStore(): { setWorktreeMeta: (worktreeId: string, meta: Partial<WorktreeMeta>) => void } | null
  resolveWorktreeSelector(selector: string): Promise<ResolvedRuntimeLinearWorktree>
  listResolvedWorktrees(): Promise<ResolvedRuntimeLinearWorktree[]>
...
export class RuntimeLinearCommands {
  ...
  async linearResolveCurrentIssue(
    ...
    let worktree: ResolvedRuntimeLinearWorktree | null = null
    ...
    if (!link.workspaceId) {
      ...
      if (backfill?.workspaceId) {
        this.host.getStore()!.setWorktreeMeta(worktree.id, {
  ...
  private async resolveWorktreeForContainedPath(
    cwd: string
  ): Promise<ResolvedRuntimeLinearWorktree | null> {
    const currentPath = resolve(cwd)
    let best: ResolvedRuntimeLinearWorktree | null = null
