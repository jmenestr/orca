/* eslint-disable max-lines -- Why: hosted-review RPC dispatch stays in one adapter boundary so GitHub/GitLab forge wiring remains easy to scan. */
import type {
...
export type ResolvedRuntimeHostedReviewWorktree = Worktree

export type RuntimeHostedReviewCommandHost = {
  resolveRepoSelector(selector: string): Promise<Repo>
  resolveWorktreeSelector(selector: string): Promise<ResolvedRuntimeHostedReviewWorktree>
  requireStore(): Store
  getStore(): Store | null
  getStats(): StatsCollector | undefined
}

export class RuntimeHostedReviewCommands {
  constructor(private readonly host: RuntimeHostedReviewCommandHost) {}
  ...
  getLocalGitExecutionOptionArgs(repo: Repo): [] | [{ wslDistro?: string }] {
    const localGitOptions = getLocalProjectWorktreeGitOptions(this.host.requireStore(), repo)
  ...
  async getHostedReviewForBranch(args: {
    ...
    if (review?.provider === 'github') {
      ...
      if (stats && !stats.hasCountedPR(review.url)) {
        stats.record({
          type: 'pr_created',
          at: Date.now(),
          repoId: repo.id,
  ...
  async createHostedReview(
    ...
    if (result.ok) {
      const stats = this.host.getStats()
      if (stats && !stats.hasCountedPR(result.url)) {
        stats.record({
          type: 'pr_created',
          at: Date.now(),
          repoId: repo.id,
