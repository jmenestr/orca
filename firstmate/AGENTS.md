...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, worktree=, project=, harness=, kind=, mode=, yolo=; kind=secondmate also records home= and projects=; mode=codespace also records codespace=, remote_worktree=, remote_state=; mode=orca also records engine=orca, worktree_path=, terminal=, repo= with an empty window= (fm-pr-check appends pr=)
  <id>.check.sh      optional slow poll per task (e.g. merged-PR check); you write it by hand, or fm-spawn auto-generates one for a mode=codespace task that SSHes in to read the remote status file, or for a mode=orca task that reads the Orca agent status via the `orca` CLI
...
- `orca` - crewmate runs INSIDE the Orca app via the `orca` CLI (no tmux pane, no local clone). The registry line carries an Orca repo selector and an optional harness (`[orca <repo-selector> [<harness>]]`, e.g. `[orca name:widget cursor]`, default harness `claude`); `fm-spawn.sh` reads them via `fm-project-mode.sh --orca-selector` / `--orca-harness`. Spawn runs an `orca status` preflight, then `orca worktree create --repo <sel> --name fm-<id> --agent <harness> --prompt "$(cat brief)" --json`, records `engine=orca`, `mode=orca`, `worktree=id:<wid>`, `worktree_path=`, `terminal=`, and `window=` (empty) in meta, and generates a `state/<id>.check.sh` that mirrors the Orca agent status into `state/<id>.status`. No per-harness turn-end hook is installed - Orca's agent status replaces it. Steering (`fm-send.sh`) and peeking (`fm-peek.sh`) route through `orca terminal send/read` on `engine=orca`; teardown runs the verbatim dirty/unpushed check on `worktree_path`, then `orca worktree rm --worktree <sel> --force`. The whole backend mirrors the codespace adapter and lives entirely in `firstmate/bin/` (`fm-orca-lib.sh` is its shared lib). Orca-specifics:
  - **No `terminal agentStatus` command in this Orca build.** The agent status it would report (`working|permission|idle`) is sourced from `orca worktree ps --json` (each worktree's rollup `.status`: active|working|permission|done|inactive) plus `orca terminal read --json` `.status` (running|exited) for process exit. `orca_check_emit` maps these to one firstmate status line: terminal exited / worktree `done` -> `done`, `permission` -> `needs-decision` (a needs-a-steer wake), `working` -> `working`, idle (active|inactive) -> `working: ... idle`, and any failed Orca call (app down) -> `unknown` (never a faked `exited`).
  - **CLI invoker** is resolved by `fm-orca-lib.sh` in order `$FM_ORCA_CLI` -> `orca` -> `orca-dev` -> `node <orca-repo>/out/cli/index.js`; the resolved value is pinned into the generated check.sh. Build the dev CLI with `pnpm build:cli`.
  - **Remote runtime (SSH use case):** when the Orca runtime is remote, `worktree_path` is a remote path. Teardown's local git check then cannot run and REFUSES (refuse-don't-infer), exactly like the codespace path; the literal clean no-force teardown applies when Orca runs locally. `FM_ORCA_BASE_BRANCH` optionally pins the worktree base ref when the repo default cannot be refreshed offline.
...
- **Ship** (the default): the deliverable is a change to the project. It ships through the project's delivery mode: `no-mistakes`, `direct-PR`, `local-only`, `codespace`, or `orca`.
